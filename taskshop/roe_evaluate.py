...
from utils import combine_scores, get_task2dir, lr2str, roe2dir, SEED_LIST
...
def evaluate_selection(target_task, source_task_list, task2dir_transfer, config_dir, roe_info, transfer_info=None, regret_k=None):
    """Evaluate the overall ranking and top-k precision of RoE (Retrieval-of-Experts).
    ...
    Args:
        ...
        task2dir_transfer (dict[str -> str]): dictionary mapping each task to its directory with transfer scores
        config_dir (str): path to directory with json files containing model hyperparameter configs
        roe_info (dict[str]): dictionary containing information about RoE (Retrieval-of-Experts) settings
    ...
    """
    # load RoE predictions
    with open(roe2dir(roe_info), "rb") as f:
    ...
    # order RoE predictions and actual pairwise transfer scores from most to least similar
    ...
    if regret_k:  # compute regret @ k
        ...
        # compute average top-5 score according to RoE predictions
...
if __name__ == "__main__":
    ...
    for category, task_list_custom in category2task.items():
        ...
        for target_task in task_list_custom:
            ...
            score, pred_rankings, gold_rankings = evaluate_selection(target_task, source_task_list, task2dir_transfer, CONFIG_DIR, roe_info, transfer_info)
            score_topk, pred_rankings, gold_rankings = evaluate_selection(target_task, source_task_list, task2dir_transfer, CONFIG_DIR, roe_info, transfer_info, regret_k=5)
            score_avg += score

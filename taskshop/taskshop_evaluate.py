...
from utils import combine_scores, get_task2dir, lr2str, taskshop_llm2dir, taskshop_retriever2dir, SEED_LIST

PAIRWISE_TRANSFER_DIR = "../data/pairwise_transfer"
SCORE_DIR = "../transfer/outputs_1000"

def load_scores(source_task, target_task, config_dir, model="t5l", adaptation="finetune"):
    ...
    Args:
        ...
        config_dir (str, optional): path to directory with json files containing model hyperparameter configs
...
def evaluate_selection(target_task, source_task_list, taskshop_info, transfer_info=None, regret_k=None, method="llm", lmbda=0.7):
    ...
    pred_path = taskshop_llm2dir(taskshop_info, transfer_info, lmbda=lmbda) if method == "llm" else taskshop_retriever2dir(taskshop_info, transfer_info, lmbda=lmbda)
    ...
    if regret_k:  # compute regret @ k
        ...
        max_label_list =  [load_scores(source_task, target_task) for source_task in gold_ranking[:regret_k]]
        max_label = sum(max_label_list) / len(max_label_list)
        # compute average top-5 score according to TaskShop predictions
        score_list = [load_scores(source_task, target_task) for source_task in pred_ranking[:regret_k]]
...
if __name__ == "__main__":
    ...
    for category, task_list_custom in category2task.items():
        ...
        for target_task in task_list_custom:
            ...
            score, pred_rankings, gold_rankings = evaluate_selection(target_task, source_task_list, taskshop_info, transfer_info, method=args.task_selection, lmbda=lmbda)
            score_topk, pred_rankings, gold_rankings = evaluate_selection(target_task, source_task_list, taskshop_info, transfer_info, precision_k=5, method=args.task_selection, lmbda=lmbda)
            score_avg += score
